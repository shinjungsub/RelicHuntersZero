...
protected:
	...
	Vector2 _mainRectPos;	//다른 대상과 충돌 감지할 렉트 좌표
	Vector2 _hitRectPos;	//총알 충돌 감지 렉트 좌표
	Vector2 _mainSize;		//메인렉트사이즈
	Vector2 _hitSize;		//히트박스사이즈
	Vector2 _imageSize;		//이미지사이즈
	POINT	_index;			//오브젝트 위치 인덱스
	RECT _mainRect;			//메인 렉트
	RECT _hitRect;			//충돌 감지 렉트
	int _hp;				//체력
